...
def main(in_csv, out_csv, logfile, name_title, title_string):
    ...
    # transfer InChiKey to InChiKey14
    inchikey_ls = [inchi[:14] for inchi in inchikey_ls]
    ...
    df_unique = pd.concat([df_unique, pd.Series(['Unique'] * df_unique.shape[0], name='Flatten')], axis=1)
    ...
    for idx in repeat_id:
        ...
        repeat_inchikey = repeat_sr.loc['InChiKey'][:14]
        ...
        if df_unique.loc[unique_idx, 'Flatten'] == 'Unique':
            df_unique.loc[unique_idx, 'Flatten'] = remark_str
        else:
            df_unique.loc[unique_idx, 'Flatten'] += ', ' + remark_str
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Duplicate for flatten')

    parser.add_argument('--cur_folder', type=str, default='', help='SHOULD end with /')
    parser.add_argument('--in_csv', type=str, default='unique.csv', help='unique.csv')
    parser.add_argument('--out_csv', type=str, default='unique_flatten.csv', help='unique_flatten.csv')
    parser.add_argument('--logfile', type=str, default='unique_flatten.log', help='unique_flatten.log')
    ...
    # execute
    main(in_csv, out_csv, logfile, name_title, title_string)
